...
Chat:
  mensagem do usuário → ChatService → ChatOrchestrator
    ...
    → TaskTool | VaultSearchTool → Service → Repository → Supabase
...
| Limpeza do cache React Query no logout                         | **Pendente para o hardening de Auth.** O isolamento primário permanece no token autenticado e no RLS; a limpeza explícita do cache de UI entre sessões será tratada na fase de Auth, sem ampliar o escopo da P3.                                                                                                              |
